...
<div >
  ...
  <table class="table " id="sectionTables">
    <thead>
      <tr>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <section ion class="form addSection">
          <form  enctype='multipart/form-data' action="" method="POST">
            <div class="form-group">
              <label for="c_name">Section Name:</label>
              <input type="text" class="form-control" name="c_name" required>
            </div>
            <div class="form-group">
              <button type="submit" class="btn btn-success" name="upload" style="height:40px">Add Section</button>
            </div>
          </form>
       </section>
        </div>
...
<script>
    $(document).ready(function(){
        $('#sectionTables').DataTable();
        
        
    });


    document.querySelector(".addSection form").addEventListener("submit", function(event) {
    event.preventDefault();
   let xhr =  new XMLHttpRequest();
   xhr.open("POST", "./controller/addCatController.php", true);
   xhr.onload = ()=>{
